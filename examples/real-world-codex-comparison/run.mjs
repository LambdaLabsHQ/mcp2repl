...
function renderPrompt(mode) {
  const modeInstructions = {
    ...
    repl: [
      ...
      "Do not call tools.navigate_page directly. In the harness, include a small generic helper that first calls api.callTool('chrome-devtools','list_pages', {}), selects the visible Apple page with api.callTool('chrome-devtools','select_page', { pageIdx }), then calls api.callTool('chrome-devtools','navigate_page', { url }). This visible-page binding is required before every navigation.",
      "Hard interaction budget: read at most the first 80 lines of the skill, write one concise harness file, run one multi-line -e command. If it fails syntax/runtime validation, make at most one focused patch and rerun one multi-line -e command.",
      "Harness size budget: keep the task harness under 180 lines. Do not build a broad resilient scraper, configurator engine, click automation library, compare-table parser framework, or large targeted-repair loop. Use simple line-based extraction from visible page text and controls.",
      "The harness itself must not assume a fixed Apple DOM. It must probe page text and controls internally, extract compact evidence, and return compact checkpoints/final JSON. Wrap uncertain tool outputs with api.unwrap(...).",
      ...
      "Implement final() so it validates every required field. If any scenario is missing price, chip, memory, storage, or two evidence facts, final() may run one targeted evaluator-side extraction pass; it must not return invariantPassed:false as the final answer unless Apple pages are inaccessible or Apple no longer exposes a public field.",
      "For this Apple task, final() may use compare/product/buy page text together. Do not spend time implementing structural compare dropdown automation unless simple line extraction fails.",
